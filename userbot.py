...
async def init_db():
    ...
    try:
        ...
        # Fixed: Remove await from close() call
        mongo_client.close()
...
class ForwardBot:
    def __init__(self):
        ...
        self.db = self.mongo_client.auto_forward_bot
    ...
    async def load_channel_pairs(self):
        try:
            cursor = self.db.channel_pairs.find({})
    ...
    async def load_message_mapping(self):
        try:
            cursor = self.db.message_mapping.find({})
            async for doc in cursor:
    ...
    async def save_message_mapping(self, source_message_id, source_channel_id, target_message_id, target_channel_id):
        try:
            await self.db.message_mapping.update_one(
                {
    ...
    async def delete_message_mapping(self, source_message_id, source_channel_id, target_channel_id=None):
        try:
            ...
            await self.db.message_mapping.delete_many(query)
    ...
    async def add_channel_pair(self, source_id, target_id, filter_keywords=None):
        try:
            await self.db.channel_pairs.insert_one({
                "source_id": source_id,
                "target_id": target_id,
    ...
    async def update_channel_filter(self, source_id, target_id, filter_keywords):
        try:
            await self.db.channel_pairs.update_one(
    ...
    async def remove_channel_pair(self, source_id, target_id=None):
        try:
            ...
            await self.db.channel_pairs.delete_many(query)
    ...
    async def is_owner(self, user_id):
        try:
            doc = await self.db.admins.find_one({"user_id": user_id, "is_owner": True})
            return doc is not None
        except Exception as e:
    ...
    async def get_edits_and_links(self):
        try:
            edits_cursor = self.db.edits.find({})
            links_cursor = self.db.links.find({})
...
async def remove_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await bot.db.edits.delete_one({"old_text": old_text})
...
async def remove_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await bot.db.links.delete_one({"old_link": old_link})
...
async def show_edits(update: Update, context: ContextTypes.DEFAULT_TYPE):
    edits_cursor = bot.db.edits.find({})
    links_cursor = bot.db.links.find({})
...
async def reset_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await bot.db.channel_pairs.delete_many({})
    await bot.db.edits.delete_many({})
    await bot.db.links.delete_many({})
    await bot.db.settings.delete_many({})
    await bot.db.message_mapping.delete_many({})
    ...
    for setting in default_settings:
        await bot.db.settings.insert_one(setting)
