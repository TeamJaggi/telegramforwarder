...
@admin_required
async def remove_edit(update: Update, ContextTypes.DEFAULT_TYPE):
    try:
        if not context.args:
...
async def setup_webhook():
    ...
    try:
        ...
        webhook_url = f"https://telegramforwarder-5gl5.onrender.com/webhook"
        ...
        await application.bot.set_webhook(
            url=webhook_url,
            allowed_updates=["message", "edited_message", "deleted_message"]
        )
...
async def main():
    ...
    for sig in (signal.SIGINT, signal.SIGTERM):
        asyncio.get_event_loop().add_signal_handler(
            sig, lambda s, f: asyncio.create_task(signal_handler(s, f))
        )
